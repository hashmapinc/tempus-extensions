...
#Local Testing
if [ "$5" == "local" ]; then
spark-submit --master local[4] --driver-class-path "$CLASSPATH" --driver-java-options "-Dlog4j.configuration=file:$4" --class $2 $1 --props $3
fi

#Live CLuster
if [ "$5" == "yarn" ]; then
spark-submit    --master yarn \
		--files /usr/hdp/current/spark-client/conf/hive-site.xml,/usr/hdp/current/hbase-client/conf/hbase-site.xml \
		...
		--driver-java-options "-Dpid=$$ -Dlog4j.configuration=file:$4" \
		--class $2 \
		$1 --props $3
fi
